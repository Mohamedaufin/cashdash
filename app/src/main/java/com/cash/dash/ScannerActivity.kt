...
class ScannerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 🚀 BACKGROUND SYNC: Pull fresh data from cloud after boot
        FirestoreSyncManager.pullDataFromCloud(this) { success, profileExists, isAdminDeleted, _ ->
            if (success && !profileExists) {
                // Profile gone -> Redirect to Entry
                val intent = Intent(this, EntryActivity::class.java)
                intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
                startActivity(intent)
                finish()
            } else if (success && isAdminDeleted) {
                // Explicitly banned
                val intent = Intent(this, EntryActivity::class.java)
                intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
            ...
            // If it fails (offline), we just stay in the app with local cache - perfect for slow internet!
        }
    }
