        ...
        Thread {
            try {
                ...
                db.collection("users").document(uid).collection("config").document("history")
                    .set(historyPayload, SetOptions.merge())

                // 5. CategoryWeekData
                db.collection("users").document(uid).collection("config").document("analytics")
                    .set(hashMapOf("CategoryWeekData" to categoryWeekPrefs.all), SetOptions.merge())

                // 6. ScannerHistory
                db.collection("users").document(uid).collection("config").document("history_scanner")
                    .set(hashMapOf("ScannerHistory" to scannerHistPrefs.all), SetOptions.merge())

                // 7. LocalScanPrefs
                db.collection("users").document(uid).collection("config").document("undo_details")
                    .set(hashMapOf("LocalScanPrefs" to localScanPrefs.all), SetOptions.merge())
